  ...
  <div class="label-management">
    <el-dialog :title="title" :visible.sync="dialogLabelVisible" :close-on-click-modal="false" :show-close="false">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button v-if="!isManagement" type="primary" @click="addLabel">保 存</el-button>
...
<script>
  ...
  export default {
    ...
    props: ['ty', 'gwid', 'selectedLabel','isManagement', ],
    data(){
      return {
    ...
    computed: {
      title(){
        return this.$props.isManagement ? '标签管理' : '添加标签';
      }
    },
    mounted(){
    ...
    methods: {
      ...
      addLabel(){
        var that = this;
        let gwid = (this.$props.gwid).toString();
        var labelid = base64.encode(JSON.stringify(that.checkModel))
        this.$jquery.post(that.$request, {
          action: 'gateway_label_add',
          admin_id: that.admin_id,
          labelid: labelid,
          gwid: gwid
        }, res => {
