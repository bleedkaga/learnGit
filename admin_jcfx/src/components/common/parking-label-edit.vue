<template>
  <div class="label-management">
    <el-dialog :title="title" :visible.sync="dialogEditLabelVisible" :close-on-click-modal="false" :show-close="false" >
      <div class="label_content_list parking-label-setting">
        <ul class="label_head">
          <li><input type="checkbox" id="check" v-model="checked" v-on:click="checkedAll"><span class="input_checkbox_inner"></span> <label for="check">全选</label> <i class="el-icon-delete" @click="deleteLabel"></i></li>
        </ul>
        ...
        <el-input placeholder="请输入要添加的标签名称" v-model="labelName">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button  type="primary" @click="cancel">保 存</el-button>
      </span>
...
<script>
  ...
  export default {
    name : 'parkingLabelEdit',
    props: ['ty','cid', 'selectedLabel'],
    data(){
      return {
        dialogEditLabelVisible:true,
        checkAll: true,
        checkedCities: [],
        cities: cityOptions,
    ...
    methods: {
      ...
      //   }, res => {
      //     if( res.error_code == '0'){
      //       this.$message({type: 'success', message: '车位标签修改成功'})
      //       this.dialogEditLabelVisible = false
      //       this.$emit('cancel')
      //     } else {
      //       this.$message({type: 'error', message: res.msg})
      //     }
