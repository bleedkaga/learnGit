  ...
  <div class="label-management">
    <el-dialog :title="title" :visible.sync="dialogLabelVisible" :close-on-click-modal="false" :show-close="false">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button v-if="!loid" type="primary" @click="saveLabel">保 存</el-button>
...
<script>
  ...
  export default {
    ...
    props: ['ty', 'loid', 'selectedLabel','isManagement', ],
    data(){
      return {
    ...
    computed: {
      title(){
        return this.$props.loid ? '添加标签' : '标签管理';
      }
    },
    mounted(){
    ...
    methods: {
      ...
      addLabel(){
        var that = this;
        let loid = (this.$props.loid).toString();
        var labelid = base64.encode(JSON.stringify(that.checkModel))
        this.$jquery.post(that.$request, {
          action: 'parking_lock_label',
          admin_id: that.admin_id,
          labelid: labelid,
          loid: loid
        }, res => {
